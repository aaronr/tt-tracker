$(document).ready(function() {
    ...
    var process_rows = function (databack) {
        newrow = [];
        $.each(databack.feed.entry, function(index, value) {
            // Grab the people from the spreadsheet
            newrow.push(value.content.$t);
    ...
    var header_url = "http://spreadsheets.google.com/feeds/cells/0AtXkFFQzkJ9xdDJ1cENXbHhWMFV2cTJ5eVhuVVdXTlE/1/public/basic?alt=json&min-row=1&max-row=1&min-col=1"
    header_url = header_url + "&callback=?";
    var row_url = "http://spreadsheets.google.com/feeds/cells/0AtXkFFQzkJ9xdDJ1cENXbHhWMFV2cTJ5eVhuVVdXTlE/1/public/basic?alt=json&min-row=2&min-col=1"
    row_url = row_url + "&callback=?";
    $.getJSON(header_url, process_headers);
    $.getJSON(row_url, process_rows);
